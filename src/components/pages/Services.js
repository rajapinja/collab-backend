...
const Card = ({ title, desc, imageUrl, link, gradientColor, onMouseEnter, onMouseLeave }) => {
  ...
  return (
    <div className="card" style={{ transition: 'transform 0.3s', background: gradientColor}} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <div>
        <h3 style={{ color: "rgb(17,137,181)", font: "bold" }}>{title}</h3>
      ...
      <div className="content">
        <hr style={{ width: '340px', borderWidth: '1px solid blue', borderColor: 'linear-gradient(to right, rgb(232, 232, 226), rgb(224, 115, 165))' }} />
        <a href={link} target="_blank" rel="noopener noreferrer" className="logo" alt={""}>{title}</a>
      </div>
    </div>
  );
};
  ...
  return (
    <div className='cardrotation'>     
      <div className={`container ${isHovered ? 'paused' : ''}`}>
        <div className="card-container left-to-right">
          {/* Left to Right Cards */}
